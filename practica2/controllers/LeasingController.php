...
class LeasingController extends ControladorBase{
	...
	public function admin(){
        ...
        $result = DaoLeasing::getInstance()->getAll();
	...
	public function crear_vehiculo()
	{
        	...
        	&& isset($_POST["combustible"]) && isset($_POST["descripcion"]) && isset($_POST["precio_dia"]))
        {
            ...
            if(empty($formErrors))
            {
                $coche = DaoLeasingCar::getInstance()->getBy("matricula", $formValues["matricula"]);

                if (!$coche) {

                    $list_img = uniqid() . "." . strtolower(end(explode('.',$_FILES['list_img']['name'])));

                    $id = DaoLeasingCar::getInstance()->insertVehiculo($formValues["matricula"], $formValues["marca"], $formValues["modelo"],
                    ...
                    if($id){
                        ...
                        $this->redirect("leasing", "fichaVehiculo", $id);
                    }
                    else {
